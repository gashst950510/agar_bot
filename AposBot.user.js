...
function AposBot() {
    ...
    this.isFood = function(player, cell){
        return (!this.isVirus(cell) && this.compareSize(cell, player, 1.33) || (cell.size <= 13));
    };
    ...
    this.version1 = function(){
        var player = getPlayer();
        if (player.length > 0) {
            var cellList = getCellsArray();
            var direct = [0, 0];
            for(var i = 0; i < cellList.length; i++){
                if(this.isItMe(player, cellList[i]))continue;
                var x_uni = cellList[i].x - player[0].x;
                var y_uni = cellList[i].y - player[0].y;
                var len = this.computeDistance(player[0].x, player[0].y, cellList[i].x, cellList[i].y, player[0].size, cellList[i].size);
                var u_len = Math.sqrt(x_uni * x_uni + y_uni * y_uni);
                x_uni/= u_len;
                y_uni/= u_len;
                if(this.isVirus(cellList[i])){
                    direct[0]+= x_uni * -1 * player[0].size / (len * len);
                    direct[1]+= y_uni * -1 * player[0].size / (len * len);
                }
                else if(this.isFood(player[0], cellList[i])){
                    direct[0]+= x_uni * cellList[i].size * 1 / (len * len);
                    direct[1]+= y_uni * cellList[i].size * 1 / (len * len);
                }
                else if(this.isThreat(player[0], cellList[i])){
                    direct[0]+= x_uni * cellList[i].size * -1 / (len * len);
                    direct[1]+= y_uni * cellList[i].size * -1 / (len * len);
                }
            }
            var d_len = Math.sqrt(direct[0] * direct[0] + direct[1] * direct[1]);
            direct[0]/= d_len;
            direct[1]/= d_len;
            //console.log(direct[0], direct[1]);
            drawLine(player[0].x, player[0].y, player[0].x + direct[0] * 500, player[0].y + direct[1] * 500, 5);
